...
function TimeHelper.addFrame ()
  if TimeHelper.frameInfo[TimeHelper.frame] then -- 如果当前帧信息存在，则清除，过去的帧信息没用了
...
function TimeHelper.addHour (hour)
  ...
  if hour >= 24 then -- 超过23点，又从0开始。这里可以取模
    hour = hour - 24
  end
...
function TimeHelper.setHour (hour)
  if WorldHelper.setHours(hour) then -- 设置当前几点成功。以前写的，后来发现这里一定会成功。
    TimeHelper.hour = hour
    return true
  end
  return false
...
function TimeHelper.getHour ()
  if not TimeHelper.hour then -- 不存在，表示还没有取过时间
...
function TimeHelper.getFrameInfo (key)
  if not TimeHelper.frameInfo[TimeHelper.frame] then -- 帧信息不存在
    return nil
  end
...
-- 添加方法
function TimeHelper.addFn (f, time)
...
function TimeHelper.delFn (time, index)
  if not index then -- 不存在序数，则删除该时间对应的所有
...
-- 参数为：函数、秒。大致几秒后执行方法
function TimeHelper.callFnAfterSecond (f, seconds)
  if type(f) ~= 'function' then -- 不是函数，则不再执行
    return
  end
  seconds = seconds or 1
  local time = TimeHelper.time + seconds
  local index = TimeHelper.addFn(f, time)
  return time, index
end
...
-- 添加方法
function TimeHelper.addFnContinueRuns (f, seconds, t)
  TimeHelper.fnContinueRuns[t] = { seconds * 1000, f}
end

-- 删除方法
function TimeHelper.delFnContinueRuns (t)
  if TimeHelper.fnContinueRuns[t] then -- 存在
...
-- 参数为：函数、秒、绑定类型，精确到0.05秒
function TimeHelper.callFnContinueRuns (f, seconds, t)
  if type(f) ~= 'function' then
    return
  end
  seconds = seconds or 1
  t = t or TimeHelper.getGlobalIndex()
  TimeHelper.addFnContinueRuns(f, seconds, t)
  return t
end
